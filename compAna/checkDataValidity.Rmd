...
final_folder="/Users/alejandrinacristia/corpora/namvanj/"
...
plot(sumchi$prop~sumchi$age,cex=(sumchi$tot/max(sumchi$tot,na.rm=T)))
text(sumchi$age,sumchi$prop,as.character(sumchi$child),cex=.5,pos=4)

#plotmeans(sumchi$age,sumchi$prop,ylim=c(0,1),n.label=F,bars=F,col="white")
#for(kid in levels(factor(sumchi$child))) plotmeans(sumchi$prop~sumchi$age,subset=c(sumchi$child==kid),add=T,n.label=F,barcol="gray")

plot(sumchi$prop~sumchi$age,type="n")
for(kidag in levels(factor(sumchi$chag))){ 
  age=mean(sumchi$age[sumchi$chag==kidag],na.rm=T)
  prop=mean(sumchi$prop[sumchi$chag==kidag],na.rm=T)
  err=sd(sumchi$prop[sumchi$chag==kidag],na.rm=T)
  text(age,prop,substr(kidag,1,3))
  lines(c(age,age),c(prop-err,prop+err),col="gray")
}

for(kid in levels(factor(sumchi$child))){ 
  props = aggregate(sumchi$prop[sumchi$child==kid],by=list(sumchi$age[sumchi$child==kid]),mean,na.rm=T)
...
plot(sumchi$lingRatio~sumchi$age,cex=(sumchi$lingRatio/max(sumchi$lingRatio,na.rm=T)))
text(sumchi$age,sumchi$lingRatio,as.character(sumchi$child),cex=.5,pos=4)

plot(sumchi$nonlingRatio~sumchi$age,cex=(sumchi$nonlingRatio/max(sumchi$nonlingRatio,na.rm=T)))
text(sumchi$age,sumchi$nonlingRatio,as.character(sumchi$child),cex=.5,pos=4)
