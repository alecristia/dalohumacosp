...
#be sure that demodata has the "key" column!

merge(x=everything,y = demo_data[ , c("key",  "age_mo", "sex", "n_of_siblings", "oder_of_birth","half_of_day_delivery"),], all.x=T,all.y=F)->everything
#there should be 5202 rows
...
Apply the same pipeline for all the file for the measure of cds

```{r calculation for "cds"}

read.table("line_per_segment.txt", header=T) -> everything

#remove all the rows that don't include the child in the conversation
cds = everything[grep("CHI",everything$addressee, value=F),]

table(cds$speakerID)

# because you are interested in the input of that children is supposely address to , you exclude SP 
cds=cds[-grep("SP",cds$speakerID, value=F),]
...
##the same but only for daylight recordings
cds[cds$half_of_day_delivery=="M",] -> cds


#DAY : create table with sum of FA1-2-3 MA 1-2 UC1-2 XOL & SP duration of vocalizations
aggregate(cds$dur, by=list(cds$child), sum, na.rm=T) -> xxx
names(xxx)<-c("child", "dur")
...
#merging both
merge(x=xxx, y=cds[ , c("child", "min_coded", "mot", "oder_of_birth", "n_of_siblings", "sex", "age_mo")], all.x=T,all.y=F) -> tot_suminput_cds
tot_suminput_cds <- unique(tot_suminput_cds)

#convert min in seconds
tot_suminput_cds$sec_coded <- tot_suminput_cds$min_coded * 60

# Number of seconds per minute containing input (overlapping!! no counting less) 
tot_suminput_cds$secspermin <- tot_suminput_cds$dur * 60 / tot_suminput_cds$sec_coded # multiply by 60 to have in minutes

#Approximation for a cds -- cdslong recording right?
tot_suminput_cds$hoursperday <- tot_suminput_cds$secspermin * 10 / 60



#merge(tot_suminput_cds, ages, by="child", all.x = T) -> tot_suminput_cds
...
# tot_suminput$agecat[tot_suminput$age_mo >48   ] <- 5 #under or equal to  12 mo
# DAY: group ages by categories
tot_suminput_cds$agecat[tot_suminput_cds$age_mo <= 12 ] <- 1 #under or equal to  12 mo
tot_suminput_cds$agecat[tot_suminput_cds$age_mo >12 & tot_suminput_cds$age_mo <= 24   ] <- 2 #under or equal to  12 mo
tot_suminput_cds$agecat[tot_suminput_cds$age_mo >24 & tot_suminput_cds$age_mo <= 36  ] <- 3 #under or equal to  12 mo
tot_suminput_cds$agecat[tot_suminput_cds$age_mo >36 & tot_suminput_cds$age_mo <= 48   ] <- 4 #under or equal to  12 mo
tot_suminput_cds$agecat[tot_suminput_cds$age_mo >48   ] <- 5 #under or equal to  12 mo





write.table(tot_suminput_cds,"~/Documents/tsimane_project/dalohumacosp/derivedFiles/tot_suminput_cds.txt",row.names=F,sep="\t")
...
```{r plots time}
ggplot(tot_suminput_day, aes( 
   x=age_mo, y= secspermin, color=as.factor(mot), size=min_coded )) +  geom_jitter(
     aes(x=age_mo))  + geom_jitter( data=tot_suminput_cds, shape=3) + labs(x="Age in Months", y="Minutes per Hour")  +  geom_abline(
           mapping = NULL, data = NULL, slope =-0.1988568 , intercept = 22.1909240,na.rm = FALSE, show.legend = NA) 
